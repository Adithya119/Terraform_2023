...
resource "aws_instance" "web-vms" {
    count = 1                                  # creating multiple resources required "count" & [count.index]  That's it.
...
resource "null_resource" "install_and_run_spc"{          # spaces are not accepted between words (should be like "install_and_run_spc"), no special characters.
    ...
    provisioner "file" {
        source = "./module/deploy-spc.sh"                         # file present on local. change the build_id everytime you modify the contents of this file.
