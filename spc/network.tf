...
resource "aws_security_group" "Web-SG" {
    vpc_id = aws_vpc.primary-vpc-2.id
    name = "Web-SG"
    description = "Allow spc, ssh, http & https traffic"  
    
    ingress {
        description = "Allow traffic on port 8080"        # for spc
    ...
    ingress {
        description = "Allow http traffic"             # required to browse the web UI of app inside a vm
